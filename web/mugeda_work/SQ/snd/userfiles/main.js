...
document.addEventListener('WeixinJSBridgeReady', function onBridgeReady() {
    WeixinJSBridge.on('menu:share:appmessage', function (argv) {
        WeixinJSBridge.invoke('sendAppMessage', {
            "appid": "",
            "img_url": 'http://cdn-cn.mugeda.com/Schneider/v1/userfiles/small.jpg',
            "img_width": 128,
            "img_height": 128,
            "link": 'http://cdn-cn.mugeda.com/Schneider/v1/index.html',
            "desc": '激情时刻，怎能断电？APC祝您赢得京东世界杯之旅，欢乐时光不断电！',
            "title": 'APC狂欢世界杯 精彩不断电-京东商城'
        });
    });
...
*/
	...
	lazyNode.each(function(){
		...
		if(self.is('img')){
			...
			self.css({
				'background-image'	: 'url(./userfiles/loading_large.gif)',
...
*/
	...
	function changeOpen(e){
		...
		$('.swiper').on('mousedown touchstart',page_touchstart);
		$('.swiper').on('mousemove touchmove',page_touchmove);
		$('.swiper').on('mouseup touchend',page_touchend);
...
*/
	function success(){
		...
		*/							
		...
		$('#arrow_up').css("display", page_n == Msize ? "none" : "block");
		$('#arrow_down').css("display", page_n == 1 ? "none" : "block");
