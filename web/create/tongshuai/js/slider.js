 ...
 */
		...
		Slider.prototype.bindDOM = function(){
			...
			function getRandom(){
				return Math.random().toString().match(/0\.(\d+)/)[1];
			}
			
			var currentKey=getRandom();
			
			function isCurrent(e){
				var target=e.target;
				if(target.tagName.toLowerCase()==="img")
					target=target.parentNode;
				return target.classList.contains("zindex");
			}
			
			//手指按下的处理事件
			var startHandler = function(evt){
				self.isMove=false;
				if(window.skey!==undefined && window.skey!==currentKey || !isCurrent(evt)) return;
				//记录手指按下的坐标
			...
			var endHandler = function(evt){
				...
				if (currentLi.hasClass('zindex') && (self.offsetX != 0 ||  self.offsetY !=0)) {
					...
					setTimeout(function() {
							...
							window.skey=currentKey;
					},500);
					p == i - 1 ? (t.eq(0).find("img").css("opacity", "1"),t.eq(0).addClass('zindex'),t.last().removeClass('zindex')) : (currentLi.next().find("img").css("opacity", "1"),currentLi.next().addClass('zindex').siblings('li').removeClass('zindex'));	  
					self.changeCard();
				}
			};
